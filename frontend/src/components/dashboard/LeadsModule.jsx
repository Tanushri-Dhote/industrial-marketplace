      ...
      <Box overflowX="auto" borderRadius="xl" border="1px solid" borderColor="gray.100">
        <Table variant="simple">
          ...
          <Tbody>
            {leads.map((lead) => (
              <Tr key={lead.id}>
                <Td fontSize="13px" fontWeight="600">{lead.name}</Td>
...
function LeadModal({ isOpen, onClose, onSave, lead }) {
  const [formData, setFormData] = useState({ 
    name: '', email: '', phone: '', product: '', message: '', status: 'New' 
  });

  React.useEffect(() => {
    if (lead) setFormData(lead);
    else setFormData({ name: '', email: '', phone: '', product: '', message: '', status: 'New' });
  }, [lead]);
          ...
          <HStack spacing={4}>
            ...
            <VStack align="flex-start" spacing={0}>
              ...
              <Text opacity={0.9} fontSize="15px">
                {isEditing ? 'Update lead info and status' : 'Capture a new inquiry'}
              </Text>
            </VStack>
          </HStack>
        </Box>
          ...
          <VStack spacing={8} align="stretch">
            <SimpleGrid columns={2} spacing={8}>
              <FormControl isRequired>
                ...
                <Input size="lg" h="56px" borderRadius="2xl" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
              </FormControl>
              <FormControl isRequired>
                <FormLabel fontWeight="700" fontSize="14px" color="gray.600">EMAIL ADDRESS</FormLabel>
                <Input type="email" size="lg" h="56px" borderRadius="2xl" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
              </FormControl>
            </SimpleGrid>
            <SimpleGrid columns={2} spacing={8}>
              <FormControl>
                <FormLabel fontWeight="700" fontSize="14px" color="gray.600">PHONE</FormLabel>
                <Input size="lg" h="56px" borderRadius="2xl" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} />
              </FormControl>
              <FormControl isRequired>
            ...
            <FormControl>
              <FormLabel fontWeight="700" fontSize="14px" color="gray.600">MESSAGE</FormLabel>
              <Textarea rows={6} borderRadius="2xl" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} />
            </FormControl>
          </VStack>
        </ModalBody>
      ...
      <ModalContent borderRadius="3xl" overflow="hidden" boxShadow="2xl">
        <Box bgGradient="linear(to-r, #0F172A, #1E293B)" color="white" py={8} px={8}>
          <HStack spacing={4}>
            ...
            <VStack align="flex-start" spacing={0}>
              <ModalHeader fontSize="26px" fontWeight="800" p={0}>Lead Details</ModalHeader>
              <Text opacity={0.85} fontSize="15px">{lead.name}</Text>
            </VStack>
          </HStack>
        </Box>
        <ModalCloseButton color="white" top={6} right={6} />
        <ModalBody p={10}>
          <VStack spacing={6} align="stretch">
            <SimpleGrid columns={2} spacing={6}>
              <Box>
                <Text fontSize="13px" fontWeight="700" color="gray.500">NAME</Text>
                <Text fontSize="18px" fontWeight="600">{lead.name}</Text>
              </Box>
              <Box>
                <Text fontSize="13px" fontWeight="700" color="gray.500">STATUS</Text>
                <Badge colorScheme={lead.status === 'Won' ? 'green' : 'blue'} fontSize="14px" px={4} py={1} borderRadius="full">{lead.status}</Badge>
              </Box>
            </SimpleGrid>
            <Box>
              <Text fontSize="13px" fontWeight="700" color="gray.500">EMAIL</Text>
