...
exports.register = async (request, reply) => {
	try {
		...
		if (resolvedSiteId) {
			...
			if (!website) return reply.status(404).send({ message: "The specified site was not found" });
...
exports.login = async (request, reply) => {
	try {
		...
		if (user.loginVerified) {
			...
			// Tracking: Update last login and IP
			user.lastLogin = new Date();
			user.lastIp = request.ip || request.headers["x-forwarded-for"] || "";
			await user.save();
		...
		return { message: "Verification email sent", requiresVerification: true };
	} catch (error) {
		reply.status(500).send({ message: error.message });
	}
};
